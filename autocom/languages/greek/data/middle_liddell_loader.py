...
def _strip_number_suffix(text: str) -> str:
    """Strip trailing number suffixes from headwords.

    Middle Liddell uses numbered entries for homographs (e.g., ἄν1, ἄν2).
    These numbers should be stripped for display.
    """
    if not text:
        return text
...
def _extract_genitive(note_text: str, headword: str) -> Optional[str]:
    ...
    Middle Liddell patterns:
    - "qi/_s, qi_no/s, o(," -> genitive is qi_no/s (second part)
    - "lo/gos, o(, lo/gou" -> genitive is lo/gou (third part, after article)

    The key is to identify which part is the article (o(, h(, to/) and skip it.
    """
    if not note_text:
        return None
    ...
    article_patterns = ("o(", "h(", "to/", "o(.", "h(.", "to/.")

    # Try to find genitive: it's usually the part after headword but before article
    # Pattern 1: "headword, genitive, article" (e.g., "qi/_s, qi_no/s, o(,")
    # Pattern 2: "headword, article, genitive" (less common)

    genitive = None

    # Check if parts[1] is the genitive (not an article)
    if len(parts) >= 2:
        potential_gen = parts[1].strip().rstrip(".")
        if potential_gen and potential_gen not in article_patterns:
            # Check if it looks like beta code Greek (not an article)
            if re.match(r"^[a-z)(/=\\|+_]+$", potential_gen):
                genitive = _beta_to_unicode(potential_gen)
                # Extract just the ending (last few characters after stem)
                if genitive and len(genitive) > 2:
                    genitive = "-" + genitive[-3:] if len(genitive) >= 3 else "-" + genitive
                return genitive

    # Pattern 2: parts[1] is article, parts[2] is genitive
    if len(parts) >= 3:
        if parts[1].strip().rstrip(".") in article_patterns:
            potential_gen = parts[2].strip().rstrip(".")
            if potential_gen and re.match(r"^[a-z)(/=\\|+_]+$", potential_gen):
                genitive = _beta_to_unicode(potential_gen)
                if genitive and len(genitive) > 2:
                    genitive = "-" + genitive[-3:] if len(genitive) >= 3 else "-" + genitive
                return genitive
...
def _get_raw_sense_text(sense_elem: ET.Element) -> str:
    """Get raw text content from sense element including direct text.

    Used for POS detection where we need to check for 'Particle', 'adv.', etc.
    """
    parts = []
    if sense_elem.text:
...
def _extract_pos_from_entry(entry_elem: ET.Element) -> str:
    ...
    if orth_elem is not None and orth_elem.text:
        ...
        if orth.endswith(("w", "mi", "mai", "omai")):
            ...
            if sense_elems:
                first_sense = _get_raw_sense_text(sense_elems[0])
                # Verb indicators in definition
                if any(ind in first_sense.lower() for ind in
                       ["to ", "pass.", "act.", "mid.", "aor.", "fut.", "perf."]):
                    return "verb"

    # Check for POS indicators in raw sense text (includes prose like "Particle used...")
    sense_elems = entry_elem.findall(".//sense")
    if sense_elems:
        # Check first few senses for POS markers (some entries have empty first sense)
        for sense_elem in sense_elems[:3]:
            sense_raw = _get_raw_sense_text(sense_elem)
            lower_sense = sense_raw.lower()
...
def _get_sense_text(sense_elem: ET.Element) -> str:
    """Extract primary translation from a sense element.

    Middle Liddell senses can contain multiple <trans><tr> elements with
    various glosses, examples, and usage notes. We extract only the primary
    translations (first few <tr> elements) for a clean definition.
    """
    translations = []

    # Collect all translations from <trans><tr> and direct <tr> elements
    for child in sense_elem:
        if child.tag == "trans":
            ...
            if child.text and child.text.strip():
                translations.append(child.text.strip())
        elif child.tag == "foreign" and child.get("lang") == "la":
            # Latin translations are useful context
            if child.text and child.text.strip():
                translations.append(f"Lat. {child.text.strip()}")

    # Limit to first 3 translations for cleaner output
    if translations:
        return ", ".join(translations[:3])

    # Fallback: get direct text content (e.g., for entries with prose definitions)
    if sense_elem.text:
        text = sense_elem.text.strip()
        # Clean up by removing references and usage markers
...
def load_middle_liddell_vocabulary() -> Dict[str, Dict[str, Any]]:
    ...
    for entry in root.iter("entry"):
        ...
        # Convert key from beta code to Unicode and strip number suffix
        lemma = _strip_number_suffix(_beta_to_unicode(entry_key))
        if not lemma:
            continue
        ...
        if orth_elem is not None and orth_elem.text:
            ...
            canonical = _strip_number_suffix(_beta_to_unicode(orth_elem.text.strip()))
            if canonical:
                lemma = canonical
        ...
        # Handle key collisions from homographs (e.g., ἄν1 and ἄν2 both become ἄν)
        # Prefer entries with more useful data (POS detected, more senses)
        if lemma in vocabulary:
            existing = vocabulary[lemma]
            # Keep existing if it has better data
            existing_quality = (1 if existing.get("pos") else 0) + len(existing.get("senses", []))
            new_quality = (1 if pos else 0) + len(senses)
