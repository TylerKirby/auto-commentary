...
# Cache file path - save in the languages/latin directory
CACHE_FILE = os.path.join(os.path.dirname(__file__), "latin_definitions_cache.json")
...
def get_cltk_semantic_info(word: str, lemma: str = None) -> Dict[str, List[str]]:
    """
    Get semantic information from CLTK for Latin words.
...
def add_latin_dictionary_references(lemma: str) -> Dict[str, str]:
    """
...
def format_latin_for_commentary(result: Dict[str, Any]) -> str:
    """
    Format Latin definition data for commentary output.
...
def get_definition(
    ...
    if include_references:
        result["references"] = add_latin_dictionary_references(result["lemma"])

    # Format the output for commentary generation
    result["formatted_definition"] = format_latin_for_commentary(result)
...
def get_contextual_definition(word: str, context: List[str], use_morpheus: bool = True) -> Dict[str, Any]:
    """
...
def bulk_lookup(words: List[str], use_morpheus: bool = True) -> Dict[str, Dict[str, Any]]:
    """
    Bulk lookup for multiple Latin words.
...
def get_definitions_for_text(
    text: str, unique_only: bool = True, use_morpheus: bool = True
) -> Dict[str, Dict[str, Any]]:
    """
    Get definitions for all words in a Latin text.
    ...
    if unique_only:
        words = set(extract_latin_words(text))
    else:
    ...
    definitions = {}
    for word in words:
        definitions[word] = get_definition(word, use_morpheus=use_morpheus)
